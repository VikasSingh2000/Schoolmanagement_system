...
from .models import TeacherExtra, StudentExtra
...
def is_teacher(user):
    return user.is_authenticated and TeacherExtra.objects.filter(user=user).exists()  

def is_student(user):
    return user.is_authenticated and StudentExtra.objects.filter(user=user).exists()  
